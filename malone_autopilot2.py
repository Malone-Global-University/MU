...
# CONFIGURATION
# -----------------------------
GLOSSARY_JSON = "glossary_terms.json"  # JSON file path
...
def build_html(slug, term_name, department_name, course_name,
    ...
    related_terms_html = "\n".join(
        [f'        <li><a href="{item["url"]}">{item["name"]}</a></li>' for item in related_terms]
    )
...
# JSON PROCESSING
# -----------------------------
def generate_pages_from_json(json_file):
...
if __name__ == "__main__":
    generate_pages_from_json(GLOSSARY_JSON)